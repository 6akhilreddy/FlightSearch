...
public class DeleteFlights extends JApplet implements ActionListener{
	  ...
	  public DeleteFlights(){
		...
		try{
			...
			ImageIcon[] icon=new ImageIcon[range]; //for displaying image
			...
			for(i=0;rs1.next();i++){  
				...
				byte[] image = null;	//initializing byte array to null because the image in the database is of the form bytes
				image = rs1.getBytes("image");
	            img[i] = Toolkit.getDefaultToolkit().createImage(image);//convert bytes to image
				icon[i] = new ImageIcon(new ImageIcon(img[i]).getImage().getScaledInstance(150, 150, Image.SCALE_DEFAULT));//convert to ImageIcon of specifield size
				...
				l1[i]=new JLabel();
				l1[i].setIcon(icon[i]);	//setting the icon to the label
				p.add(l1[i],gbc);
				...
				b1[i].addActionListener(this);
				 ...
				 x++;
				/*
				 * for displaying the records in next row after four columns 
				 */
				 if(x==4){
				 x=0;y+=9;}
	    ...
	    f.setLayout(null);  
	    f.setVisible(true);
	    f.setExtendedState(JFrame.MAXIMIZED_BOTH);//for displaying full screeen 
...
public void actionPerformed(ActionEvent ae){
  ...
  if( ae.getSource() instanceof JButton) {
     ...
     ((JButton)ae.getSource()).setOpaque(true);	//change the color of button after click
  }
  
  	JButton source = (JButton)ae.getSource();
    int id = (int) source.getClientProperty("id"); //get id of particular button
    ...
    //database connection
    		try{
            	...
            	Statement stmt=con.createStatement();  
            	/*
            	 * sql query to delete the user ticket of particular flight and then delete the flight to avoid integrity constraint violation
            	 */
